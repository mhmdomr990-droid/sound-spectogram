...
const HOOK = `(() => {
  ...
  WebGLRenderingContext.prototype.texImage2D = function(...args) {
    ...
    try {
      let data = args[8] || args[6];
      if (data instanceof Uint8Array && args[3]*args[4] === data.length && window.__cap.length < 12) {
        const w = args[3], h = args[4];
        let nonzero = 0, max=0, sum=0;
        for (let i=0;i<data.length;i++){ const v=data[i]; if (v>0) nonzero++; if (v>max) max=v; sum+=v; }
        window.__cap.push({w,h,nonzeroPct:+(100*nonzero/(w*h)).toFixed(3),max,mean:+(sum/data.length).toFixed(4), sample:Array.from(data.slice(0,3000))});
      }
    } catch(e){}
    return res;
  };
...
const READ = `(() => JSON.stringify(window.__cap))()`;
...
async function main() {
  ...
  await ev(cdp, `(async () => {
    const r = await fetch('${BASE}/api/auth/login', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({username:'admin',password:'r7ZZqT79l57EOBv'}) });
    const j = await r.json(); if (!j.token) return 'fail';
    localStorage.setItem('token', j.token); localStorage.setItem('user', JSON.stringify(j.user||{})); return 'ok';
  })()`);
  ...
  await sleep(6000);
  const res = await ev(cdp, READ);
  const caps = JSON.parse(res);
  for (const c of caps) {
    console.log(`TEX ${c.w}x${c.h} nonzero=${c.nonzeroPct}% max=${c.max} mean=${c.mean}`);
  }
